...
import type { ITabItem } from '~/components/Process/interfaces/IProcessTabs'
...
<template>
  ...
  >
    <ul class="process-tabs__tabs">
      ...
      >
        <button
          ...
          @click="$emit('changeActiveTab', { $event, tabIndex, event: 'click' })"
        >
          <img
            loading="lazy"
            :src="tab.tabIcon.url"
    ...
    <div class="process-tabs__content-column">
      <div
        v-for="(tab, tabIndex) in tabs"
        :key="`process-tab-content-${tabIndex}`"
